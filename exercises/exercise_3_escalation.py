    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "file, behavior, or risk in the diff."
            )},
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined = llm.invoke([
            {
                "role": "system",
                "content": "Refine the pull request review using reviewer answers. Return structured PRAnalysis.",
            },
            {
                "role": "user",
                "content": (
                    f"Original summary: {initial.summary}\n"
                    f"Original confidence: {initial.confidence:.2f}\n"
                    f"Original reasoning: {initial.confidence_reasoning}\n\n"
                    f"Diff:\n{state['pr_diff']}\n\n"
                    f"Reviewer Q&A:\n{qa}"
                ),
            },
